    ...
    for (const i in WORKING_INFO) {
        ...
        describe('Mock Info ' + i, () => {
            ...
            step('all string values are correct', () => {
                ...
                strict.strictEqual(info.description, MOCK_INFO_DATA.description);
                strict.strictEqual(info.version, MOCK_INFO_DATA.version);
                strict.strictEqual(info.termsOfServiceURL, MOCK_INFO_DATA.termsOfServiceURL);
            ...
            step('should return a timestamp for created and modified', () => {
                strict.strictEqual(info.created.toISOString(), new Date(MOCK_INFO_DATA.created * 1000).toISOString());
                strict.strictEqual(info.modified.toISOString(), new Date(MOCK_INFO_DATA.modified * 1000).toISOString());
            });
